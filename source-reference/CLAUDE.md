# CLAUDE.md - PyAgentT (CLI + MCP + React Web)
...
Wrapper scripts (`pyagentt.bat` / `pyagentt`) live in the repo root so users don't need to type `.venv` paths:

```bash
# Mac / Linux (from repo root)
./pyagentt doctor
./pyagentt hot --chains solana,base --limit 10
./pyagentt new-runners-watch --chain=solana --watch-chains=solana,base --profile=discovery --max-age-hours=48 --include-unknown-age --interval=2
./pyagentt-mcp
```
...
Full `.venv` paths also work: `.\.venv\Scripts\pyagentt.exe` (Windows) or `./.venv/bin/pyagentt` (Mac/Linux).
