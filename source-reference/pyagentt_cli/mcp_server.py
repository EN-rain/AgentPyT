...
def _quickstart_paths(platform: str) -> tuple[str, str]:
    ...
    if platform == "mac-linux":
        return (str(root / ".venv" / "bin" / "pyagentt"), str(root / ".venv" / "bin" / "pyagentt-mcp"))
    return (str(root / ".venv" / "Scripts" / "pyagentt.exe"), str(root / ".venv" / "Scripts" / "pyagentt-mcp.exe"))
...
def _quickstart_prefix(platform: str) -> str:
    if platform == "windows-cmd":
        return r".\.venv\Scripts\pyagentt.exe"
    if platform == "windows-powershell":
        return r".\.venv\Scripts\pyagentt.exe"
    return "./.venv/bin/pyagentt"
...
def _quickstart_commands(platform: str, goal: str) -> list[str]:
    ...
    mcp_prefix = (
        r".\.venv\Scripts\pyagentt-mcp.exe"
        if selected_platform in {"windows-cmd", "windows-powershell"}
        else "./.venv/bin/pyagentt-mcp"
    )
    live = (
...
def _quickstart_common_mistakes(platform: str) -> list[str]:
    ...
    if selected_platform == "windows-cmd":
        return [
            ...
            "If pyagentt is not recognized, run .\\.venv\\Scripts\\pyagentt.exe from the repo root.",
        ]
    if selected_platform == "windows-powershell":
        return [
            "If pyagentt is not recognized, run .\\.venv\\Scripts\\pyagentt.exe from the repo root.",
    ...
    return [
        "If pyagentt is not recognized, activate the virtual environment or run the binary from .venv/bin.",
    ]
...
@mcp.resource("pyagentt://profiles", name="profiles", description="Recommended scan profiles.")
async def resource_profiles() -> dict[str, Any]:
    return {"profiles": SCAN_PROFILE_BASELINES, "names": list(SCAN_PROFILE_NAMES)}


@mcp.resource("pyagentt://presets", name="presets", description="Current saved scan presets.")
async def resource_presets() -> dict[str, Any]:
    store = StateStore()
    return {"count": len(store.list_presets()), "items": [p.to_dict() for p in store.list_presets()]}


@mcp.resource("pyagentt://tasks", name="tasks", description="Current saved scan tasks.")
...
@mcp.resource("pyagentt://cli-guide", name="cli-guide", description="CLI-first onboarding and troubleshooting guide.")
async def resource_cli_guide() -> dict[str, Any]:
    return {
        "recommendedLiveCommandWindows": (
            r".\.venv\Scripts\pyagentt.exe new-runners-watch --chain=solana --watch-chains=solana,base "
            r"--profile=discovery --max-age-hours=48 --include-unknown-age --interval=2"
        ),
        "recommendedLiveCommandMacLinux": (
            r"./.venv/bin/pyagentt new-runners-watch --chain=solana --watch-chains=solana,base "
            r"--profile=discovery --max-age-hours=48 --include-unknown-age --interval=2"
        ),
        "recommendedHotCommandWindows": r".\.venv\Scripts\pyagentt.exe hot --chains=solana,base --limit=10",
        "recommendedHotCommandMacLinux": r"./.venv/bin/pyagentt hot --chains=solana,base --limit=10",
        "recommendedQuickstartCommandWindows": r".\.venv\Scripts\pyagentt.exe quickstart --shell cmd --goal live",
        "recommendedQuickstartCommandMacLinux": r"./.venv/bin/pyagentt quickstart --shell bash --goal live",
        ...
        "commonMistakes": [
            ...
            "Using pyagentt before activating the environment or before using the .venv path.",
